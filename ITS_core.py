...
def main(argv):
	...
	try:
		...
		if (node_type==maps.obu_node):
			...
			t=Thread(target=obu_application_txd, args=(node_id, start_flag, my_system_rxd_queue, ca_service_txd_queue, den_service_txd_queue,))
			t.start()
			threads.append(t)
			...
			t=Thread(target=obu_system, args=(node_id, start_flag, coordinates, node_interface, my_system_rxd_queue, movement_control_txd_queue,))
			t.start()
			threads.append(t)
			...
			# 			  ca_service_txd_queue: queue to send data to ca_services_txd
			#             den_service_txd_queue: queue to send data to den_services_txd
			t=Thread(target=rsu_application_txd, args=(node_id, start_flag, my_system_rxd_queue, ca_service_txd_queue, den_service_txd_queue,))
			t.start()
			threads.append(t)
